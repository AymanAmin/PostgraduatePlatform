...
export class TypeLeaveComponent implements OnInit {
  ...
  TypeLeaveForm: FormGroup = new FormGroup({});
  ...
  constructor(private titleService: Title, private http: HttpClient, private route: ActivatedRoute, private router: Router) {
    ...
    if (this.LangCode == "en-us" || this.LangCode == "us-en")
      this.titleService.setTitle("List Type Leaves");
    else
      this.titleService.setTitle("قائمة بإنواع الإجازات");
  }

  ngOnInit() {
    this.LangCode = localStorage.getItem("LangCode");
    this.loadJsFile("assets/js/MyScript.js");
    this.getTypeLeaveList();
  ...
  CreateForm() {
    this.TypeLeaveForm = new FormGroup({
  ...
  getData() {
    this.http.get(environment.baseUrl + '/API/SystemAdmin/TypeLeaveManagment/Get/TypeLeaveInfo.ashx?GN_Code=' + this.GN_Code).subscribe(
      data => {
        var jsonInfo = JSON.stringify(data);
  ...
  OnSubmit(IsDeleted: boolean) {
    ...
    formData.append("Name_Ar", this.TypeLeaveForm.get('Name_Ar')?.value);
    formData.append("Name_En", this.TypeLeaveForm.get('Name_En')?.value);
    formData.append("CreatedBy", localStorage.getItem("GN_Code"));
    formData.append("IsActive", this.IsActive);
    formData.append("IsDeleted", IsDeleted);

    this.http.post(environment.baseUrl + '/API/SystemAdmin/TypeLeaveManagment/Set/TypeLeaveInfo.ashx', formData).subscribe(
      (response) => {
        if (response != "0") {
          if (response == "-2") {
