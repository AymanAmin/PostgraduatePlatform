...
export class ListDepartmentComponent implements OnInit {
  ...
  constructor(private titleService: Title, private http: HttpClient, private route: ActivatedRoute, private router: Router) {
    ...
    if (this.LangCode == "en-us" || this.LangCode == "us-en")
      ...
      this.titleService.setTitle("قائمة بالأقسام");
  }
  ...
  getData() {
    this.http.get(environment.baseUrl + '/API/SystemAdmin/DepartmentManagment/Get/DepartmentInfo.ashx?GN_Code=' + this.GN_Code).subscribe(
      data => {
        var jsonInfo = JSON.stringify(data);
  ...
  OnSubmit(IsDeleted: boolean) {
    ...
    formData.append("Name_Ar", this.DepartmentForm.get('Name_Ar')?.value);
    formData.append("Name_En", this.DepartmentForm.get('Name_En')?.value);
    formData.append("CreatedBy", localStorage.getItem("GN_Code"));
    formData.append("IsActive", this.IsActive);
    formData.append("IsDeleted", IsDeleted);

    this.http.post(environment.baseUrl + '/API/SystemAdmin/DepartmentManagment/Set/DepartmentInfo.ashx', formData).subscribe(
      (response) => {
        if (response != "0") {
          if (response == "-2") {
